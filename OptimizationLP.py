...
class OptimizationLP:
    ...
    # Battery power
    p_ch_est_bs = pl.LpVariable.dicts('p_ch_est_bs',range(self.Datas.NP_3TH),lowBound=0, upBound=p_max_bat_est,cat='Continuous')
    p_dc_est_bs = pl.LpVariable.dicts('p_dc_est_bs',range(self.Datas.NP_3TH),lowBound=0, upBound=p_max_bat_est,cat='Continuous')
    flag_ch_bat_est = pl.LpVariable.dicts('flag_ch_bat_est',range(self.Datas.NP_3TH),cat='Binary')
    flag_dc_bat_est = pl.LpVariable.dicts('flag_dc_bat_est',range(self.Datas.NP_3TH),cat='Binary')
    soc_est = pl.LpVariable.dicts('soc_est',range(self.Datas.NP_3TH),lowBound=soc_min_est, upBound=soc_max_est,cat='Continuous')
    # Battery absolute
    mod_est_1 = pl.LpVariable.dicts('mod_est_1', range(self.Datas.NP_3TH), lowBound= 0, upBound=1, cat='Continuous')
    mod_est_2 = pl.LpVariable.dicts('mod_est_2', range(self.Datas.NP_3TH), lowBound= 0, upBound=1, cat='Continuous')
    flag_mod_est_1 = pl.LpVariable.dicts('flag_mod_est_1', range(self.Datas.NP_3TH), cat='Binary')
    flag_mod_est_2 = pl.LpVariable.dicts('flag_mod_est_2', range(self.Datas.NP_3TH), cat='Binary')
    # Baterry variation
    mod_dif_est_1 = pl.LpVariable.dicts('mod_dif_est_1', range(self.Datas.NP_3TH), lowBound= 0, upBound=1, cat='Continuous')
    mod_dif_est_2 = pl.LpVariable.dicts('mod_dif_est_2', range(self.Datas.NP_3TH), lowBound= 0, upBound=1, cat='Continuous')
    flag_mod_dif_est_1 = pl.LpVariable.dicts('flag_mod_dif_est_1', range(self.Datas.NP_3TH), cat='Binary')
    flag_mod_dif_est_2 = pl.LpVariable.dicts('flag_mod_dif_est_2', range(self.Datas.NP_3TH), cat='Binary')
...
def run_online_optimization(constants, Input, Output, pesos, mult_bikes):
    ...
    prob = pl.LpProblem("isolated_opt_3th", pl.LpMinimize) # LpMinimize e LpMaximize
    solver = pl.PULP_CBC_CMD(msg=False, timeLimit=60*1)
